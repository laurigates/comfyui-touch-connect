...
| `web/dist/` | **Generated** — `bun build` output (`index.js`). Git-**tracked** (committed; CI gates it in sync with `src/` via `git diff --exit-code -- web/dist`) and force-shipped to the registry via `[tool.comfy] includes`. Do not edit by hand — rebuild with `bun run build` and commit. |
...
git-**tracked** (committed and CI-sync-gated against `src/`). After editing
`src/index.ts` you must `bun run build` and commit the rebuilt bundle before
